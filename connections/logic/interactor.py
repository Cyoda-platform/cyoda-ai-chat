...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(classname)s - %(message)s",
)
...
class ConnectionsInteractor:
    ...
    def send_get_request(
        self, token: str, api_url: str, path: str
    ...
    def send_post_request(self, token: str, api_url: str, path: str, data, json) -> Optional[requests.Response]:
        url = f"{api_url}/{path}"
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"{token}",
    ...
    def send_put_request(self, token: str, api_url: str, path: str, data, json) -> Optional[requests.Response]:
        url = f"{api_url}/{path}"
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"{token}",
    ...
    def ingest_data(self, token: str, request):
        ...
        ds_response = self.send_get_request(
        ...
        if ds_response and ds_response.status_code == 200:
            ...
            operation_map = {
                ...
                for endpoint in ds_data.get("endpoints", [])
            }
            ...
            endpoint_response = self.send_post_request(
            ...
            if endpoint_response and endpoint_response.status_code == 200:
                ...
                #ingestion_data = json.dumps(endpoint_response.json().get('responseContent'), indent=2)
                ingestion_data = endpoint_response.json().get('responseContent')

                print(ingestion_data)
                ingestion_response = {}
                if request.query_params.get("schema") == "true":
                    ingestion_response = self.send_post_request(
                        token=token,
                        api_url=API_URL,
                        path=f"{POST_SAVE_SCHEMA_PATH}/import/{request.query_params.get("dataFormat")}/SAMPLE_DATA/{request.query_params.get("entityName")}/{request.query_params.get("modelVersion")}",
                        data=ingestion_data,
                        json=None
                    )
                    if ingestion_response and ingestion_response.status_code == 200:
                        lock_schema_response = self.send_put_request(
                            token=token,
                            api_url=API_URL,
                            path=f"{POST_SAVE_SCHEMA_PATH}/{request.query_params.get("entityName")}/{request.query_params.get("modelVersion")}/lock",
                            data=None,
                            json=None
                        )
                        print(lock_schema_response)
                else:    
                    ingestion_response = self.send_post_request(
                        token=token,
                        api_url=API_URL,
                        path=f"{POST_SAVE_DATA_PATH}/{request.query_params.get("dataFormat")}/{request.query_params.get("entityType")}/{request.query_params.get("entityName")}/{request.query_params.get("modelVersion")}",
                        data=ingestion_data,
                        json=None
                    )
                print(ingestion_response)
                if ingestion_response and ingestion_response.status_code == 200:
                    logger.info(f"Ingestion succeeded: {ingestion_response}")

            else:
                logger.error("Failed to get a valid response from POST request")
...
"""     def ingest_data(self, token: str, data):
        ...
        if ds_response and ds_response.status_code == 200:
            ...
            for operation in data.get("data_source_operations"):
                endpoint_key = operation.get("operation")
                endpoint = operation_map.get(endpoint_key)
                connection_idx = endpoint.get("connectionIndex")
                connection = ds_data.get("connections", [{}])[connection_idx]
                endpoint_request = {
                    "type": "HTTP",
                ...
                if endpoint_response and endpoint_response.status_code == 200:
                    ...
                    ingestion_response = self.send_post_request(
                        token=token,
                        api_url=API_URL,
                        path=f"{POST_SAVE_MODEL_PATH}/{data.get("dataFormat")}/{data.get("entityType")}/{data.get("entityName")}/{data.get("modelVersion")}",
                        data=ingestion_data,
                        json=None
                    )
                    if ingestion_response and ingestion_response.status_code == 200:
            ...
            logger.error("Failed to fetch connection configuration") """
